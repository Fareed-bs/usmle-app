...
const Step1Basic = () => {
  ...
  const fetchQuestions = () => {
    setLoading(true);
    setError(null);
    axios.get("http://localhost:5000/api/step1/basic", { withCredentials: true })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/step1/basic/submit",
        { answers },
  ...
  return (
    <div style={{ padding: "2rem", fontFamily: "Arial", position: "relative", minHeight: "100vh" }}>
      ...
      <h1>USMLE Step 1 Starter Quiz</h1>
      ...
      {!quizStarted && !submitted && questions.length > 0 && (
        <div style={{ textAlign: "left", margin: "2rem 0" }}>
          <h2>Overview:</h2>
          <p>1. Designed to simulate USMLE Step 1-style questions.</p>
          <p>2. Contains 20 basic-level questions.</p>
          <p>3. Serves as a warm-up quiz before the main (core Step 1) quiz.</p>
